...
// Simple in-memory IP rate limiter to prevent anonymous credit abuse.
// Limits: 5 diagnose calls per IP per 10 minutes.
...
export const diagnoseCrop = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<DiagnoseResult> => {
    ...
    if (!checkRateLimit(ip)) {
      return {
        ok: false,
        error: "Too many diagnoses from this device. Please wait a few minutes and try again.",
      };
    }
